...
import {
  ...
  extractLiveFields,
  hasCompleteLiveData,
  buildSearchUrl as buildSearchUrlParsed,
  buildDetailUrl as buildDetailUrlParsed,
} from './dmartLiveParse.js';
import {
  initDmartAdjust,
...
function buildSearchUrl(sku, warehouseId) {
  return buildSearchUrlParsed(BFF_BASE, ENTITY, sku, warehouseId);
}

function buildDetailUrl(productId, warehouseId) {
  return buildDetailUrlParsed(BFF_BASE, ENTITY, productId, warehouseId);
}
...
function requestViaExtensionOnce(sku, warehouseId, timeoutMs) {
  return new Promise((resolve) => {
    ...
    function finish(result) {
      if (settled) return;
      if (seqAtStart !== bridgeAbortSeq) {
        settled = true;
        clearTimeout(timer);
        window.removeEventListener('message', onMsg);
        resolve({ ok: false, reason: 'aborted', onHand: null, reserved: null, price: null });
        return;
      }
      settled = true;
      clearTimeout(timer);
...
async function fetchViaLiveRelay(sku, warehouseId) {
  ...
  const seqAtStart = bridgeAbortSeq;
  const aborted = () => seqAtStart !== bridgeAbortSeq;
  try {
    if (aborted()) {
    ...
    const createTimer = setTimeout(() => createCtrl.abort(), 8000);
    let createRes;
    try {
      createRes = await fetch(SB_URL + '/rest/v1/dmart_live_requests', {
        method: 'POST',
    ...
    while (Date.now() < deadline) {
      ...
      const pollTimer = setTimeout(() => pollCtrl.abort(), 4000);
      let res;
      try {
        res = await fetch(q, {
...
export function getBridgeLastSeen() {
  return lastBridgeAt || 0;
}

function markBridgeReady(sessionOk) {
  try {
    if (sessionOk === false) {
      window.dispatchEvent(new CustomEvent('smouha:session-auth-required', {
        detail: { message: 'Extension connected but DMart session is not ready — open the branch portal page.' }
      }));
    } else if (sessionOk === true || sessionOk == null) {
      window.dispatchEvent(new CustomEvent('smouha:session-auth-ok'));
    }
  } catch (e) {}
...
function applyBridgeStatusPayload(data) {
  ...
  updateBridgeStatusUi();
  try {
    if (st === 'SESSION_EXPIRING') {
      window.dispatchEvent(new CustomEvent('smouha:session-auth-required', {
        detail: { message: 'DMart session is expiring soon — open the branch inventory page on the PC to refresh.' }
      }));
    } else if (ok) {
      window.dispatchEvent(new CustomEvent('smouha:session-auth-ok'));
    } else if (st) {
      window.dispatchEvent(new CustomEvent('smouha:session-auth-required', {
        detail: { message: 'DMart session not connected — open the portal inventory page with the extension.' }
      }));
    }
  } catch (e) {}
}
...
/* Stock adjust UI → dmartAdjust.js */
...
export function setLiveLoading(root) {
  ...
  if (status) {
    status.textContent = isBridgeOnline()
      ? 'Fetching live stock from branch…'
      : 'Connecting for live stock…';
    status.hidden = false;
  }
}
    ...
    while (stillCurrent() && Date.now() - startedAt < LIVE_MAX_WAIT_MS) {
      ...
      try {
        ...
        if (data && stopReasons.has(data.reason)) {
          ...
          if (data.reason === 'auth-required' || data.reason === 'unauthorized' || data.reason === 'no-token') {
            try {
              window.dispatchEvent(new CustomEvent('smouha:session-auth-required', {
                detail: { message: 'DMart session expired or missing — open inventory on the PC (extension) to refresh.' }
              }));
            } catch (e) {}
          }
          return;
        ...
        const status = root.querySelector('[data-live="status"]');
        if (status) {
          status.textContent = isBridgeOnline() ? 'Fetching live stock from branch…' : 'Requesting live stock (relay)…';
          status.hidden = false;
        }
        root.classList.add('is-loading');
        root.classList.remove('is-error', 'is-ready');
...
/* Bridge silence watchdog — alert UI if extension goes quiet while previously online */
const BRIDGE_SILENCE_MS = 3 * 60 * 1000; // 3 minutes
let bridgeWatchdogTimer = null;
let bridgeWasOnline = false;


/* Wire adjust module with live helpers (no circular import of bind internals) */
initDmartAdjust({
  isBridgeOnline: () => isBridgeOnline(),
  isDesktopViewport: () => isDesktopViewport(),
  setLiveValues: (root, data) => setLiveValues(root, data),
  invalidateCacheKey: (key) => { try { cache.delete(key); } catch (e) {} },
});
...
export function startBridgeWatchdog() {
  ...
  bridgeWatchdogTimer = setInterval(() => {
    try {
      ...
      if (!bridgeWasOnline) return;
      if (!lastBridgeAt) return;
      const silentFor = Date.now() - lastBridgeAt;
      if (silentFor >= BRIDGE_SILENCE_MS) {
        window.dispatchEvent(new CustomEvent('smouha:session-auth-required', {
          detail: {
            message: 'Extension silent for ' + Math.round(silentFor / 60000) + ' min — check the PC bridge and DMart tab.'
          }
        }));
        bridgeWasOnline = false; // avoid spam until it comes back
      }
    } catch (e) {}
  }, 30000);
}
